...
case "$supabase_url" in
  ...
  *) echo "M7 smoke only accepts a local SUPABASE_URL" >&2; exit 1 ;;
esac
...
docker run --rm \
  ...
  -w /workspace \
  denoland/deno:2.1.4 \
  deno test --allow-env --allow-net tests/integration/milestone_7_orchestration_integration_test.ts

if rg -n --hidden --glob '!.git/**' --glob '!*.md' --glob '!*.json' --glob '!*.env*' \
...
echo "Milestone 7 Phase 1 deterministic local smoke passed: collection claimed/completed and one intelligence job enqueued."
