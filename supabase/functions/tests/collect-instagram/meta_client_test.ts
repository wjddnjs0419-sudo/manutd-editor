...
Deno.test('builds the configured Business Discovery request and returns decoded JSON', async () => {
  ...
  assert.equal(requestedUrl?.searchParams.get('access_token'), 'meta-secret-token');
  ...
  const client = createMetaClient({
    ...
    fetch: () => {
      ...
      return Promise.resolve(
        attempts === 1
          ? jsonResponse({ error: { message: 'do not expose this body' } }, 429, {
            'retry-after': '2',
          })
...
Deno.test('retries thrown network failures and succeeds on the third attempt', async () => {
  ...
  const client = createMetaClient({
    ...
    fetch: () => {
      attempts += 1;
      return attempts < 3
        ? Promise.reject(new TypeError('socket included sensitive infrastructure details'))
...
Deno.test('does not retry permanent Meta errors and redacts secret material', async () => {
  ...
  const client = createMetaClient({
    ...
    fetch: () => {
      ...
      return Promise.resolve(
        jsonResponse(
          { error: { message: 'raw body contains meta-secret-token and private detail' } },
          401,
        ),
      );
    },
...
Deno.test('stops after three transient failures with a safe error', async () => {
  ...
  const client = createMetaClient({
    ...
    fetch: () => {
      attempts += 1;
      return Promise.resolve(jsonResponse({ error: { message: 'private detail' } }, 500));
    },
    sleep: () => Promise.resolve(),
    random: () => 0,
  });
