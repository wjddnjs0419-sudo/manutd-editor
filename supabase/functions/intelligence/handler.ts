import type { IntelligenceRunSummary } from "./types.ts";

export interface IntelligenceHandlerDependencies {
  readonly collectorSecret: string;
  readonly run: (runAt: Date) => Promise<IntelligenceRunSummary>;
  readonly now?: () => Date;
...
function safeSummary(requestId: string, result: IntelligenceRunSummary) {
  return {
...
export function createIntelligenceHandler(
  dependencies: IntelligenceHandlerDependencies,
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      const runAt = await requestedRunAt(request, now);
      const result = await dependencies.run(runAt);
      const status = result.status === "already_running" ? 202 : 200;
      log({
        requestId: id,
      ...
      return json(safeSummary(id, result), status);
    } catch (error) {
      ...
      return json(
        {
          ...
          error: {
            code: "INTELLIGENCE_FAILED",
            message: "Intelligence run failed",
          },
        },
        500,
      );
